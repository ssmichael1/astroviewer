...
/// FORCEIP and the persistent-IP / `GevCurrentIPConfiguration` registers.
/// Reached only from `examples/gev_force_ip.rs`, which includes `src/gige` via
/// `#[path]`; the viewer binary has no force-IP UI, so from its crate root the
